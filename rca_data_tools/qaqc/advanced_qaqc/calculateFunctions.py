...
def opt_calculate_ratios(optaa):
    ...
    * Q Band Ratio -- the Soret and the Q bands represent the two main
    ...
    ratio_cdom = apg[:, m412] / apg[:, m440]
    ratio_carotenoids = apg[:, m490] / apg[:, m440]
    ratio_phycobilins = apg[:, m530] / apg[:, m440]
    ratio_qband = apg[:, m676] / apg[:, m440]
...
def opt_estimate_chl_poc(optaa, site, chl_line_height=0.020):
    ...
    """
    ## load calibration coefficients and braodcast to dataset
    ...
    m650 = np.argmin(
        np.abs(coeffs["CC_awlngth"] - 650.0)
    )  # find the closest wavelength to 650 nm
    m676 = np.argmin(
        np.abs(coeffs["CC_awlngth"] - 676.0)
    )  # find the closest wavelength to 676 nm
    m715 = np.argmin(
        np.abs(coeffs["CC_awlngth"] - 715.0)
    ...
    m660 = np.argmin(
        np.abs(coeffs["CC_cwlngth"] - 660.0)
    )  # find the closest wavelength to 660 nm
    cpg = optaa["beam_attenuation"]
    estimated_poc = cpg[:, m660 - 1 : m660 + 2].median(axis=1) * 381
